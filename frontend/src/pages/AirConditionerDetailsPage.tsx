...
export default function AirConditionerDetailsPage() {
  ...
  const [selectedErrorCode, setSelectedErrorCode] = useState<ErrorCode | null>(
    null,
  );
  ...
  const {
    ...
    error: acError,
  } = useAirConditioner(id!);
  const { data: errorCodesData, isLoading: isLoadingCodes } =
    useAirConditionerErrorCodes(id!, 1, 100);
  const errorCodes = errorCodesData?.items ?? [];
  const isLoading = isLoadingAc || isLoadingCodes;
  ...
  if (isLoading) {
    return <AirConditionerDetailsSkeleton />;
  }
        ...
        <Card className="glass-card mb-8">
          <CardHeader>
            <CardTitle className="text-foreground flex items-center gap-2">
              ...
              {t(
                "air_conditioners.technical_specs",
                ...
                <div className="p-4 rounded-xl bg-background/50 border border-border">
                  ...
                  <div className="space-y-3">
                    {ac.pipe_size_liquid && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.liquid_pipe", "Liquid Pipe")}
                        </p>
                    ...
                    {ac.pipe_size_gas && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.gas_pipe", "Gas Pipe")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.pipe_size_gas}
                        </p>
                    ...
                    {ac.max_pipe_length && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t(
                            "air_conditioners.max_pipe_length",
                            "Max Pipe Length",
                          )}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.max_pipe_length}m
                        </p>
                      </div>
                    )}
                    {ac.max_height_difference && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t(
                            "air_conditioners.max_height_diff",
                            "Max Height Difference",
                          )}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.max_height_difference}m
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="p-4 rounded-xl bg-background/50 border border-border">
                  ...
                  <div className="space-y-3">
                    {ac.refrigerant_type && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.refrigerant_type", "Type")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.refrigerant_type}
                        </p>
                    ...
                    {ac.factory_refrigerant_charge && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t(
                            "air_conditioners.factory_charge",
                            "Factory Charge",
                          )}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.factory_refrigerant_charge}g
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="p-4 rounded-xl bg-background/50 border border-border">
                  ...
                  <div className="space-y-3">
                    {ac.power_supply_location && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.power_supply", "Power Supply")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.power_supply_location}
                        </p>
                    ...
                    {ac.cable_section && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.cable_section", "Cable Section")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.cable_section}
                        </p>
                    ...
                    {ac.recommended_fuse && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t(
                            "air_conditioners.recommended_fuse",
                            "Recommended Fuse",
                          )}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.recommended_fuse}A
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="p-4 rounded-xl bg-background/50 border border-border">
                  ...
                  <div className="space-y-3">
                    {ac.indoor_dimensions && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.dimensions", "Dimensions")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.indoor_dimensions}
                        </p>
                    ...
                    {ac.weight_indoor && (
                      <div>
                        <p className="text-xs text-muted-foreground">
                          {t("air_conditioners.weight", "Weight")}
                        </p>
                        <p className="text-foreground font-medium">
                          {ac.weight_indoor}kg
                        </p>
        ...
        <CardContent>
          {errorCodes.length === 0 ? (
            <p className="text-muted-foreground italic">
              No error codes registered for this unit.
            </p>
          ) : (
          ...
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              {selectedErrorCode?.severity && (
                <Badge
                  variant={
                    selectedErrorCode.severity === "Critical"
                      ? "destructive"
                      : "outline"
                  }
                >
            ...
            <DialogTitle className="text-xl">
              {selectedErrorCode?.error_name ||
                t("air_conditioners.error_details", "Error Details")}
            </DialogTitle>
          </DialogHeader>
